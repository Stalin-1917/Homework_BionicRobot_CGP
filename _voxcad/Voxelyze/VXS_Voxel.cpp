...
void CVXS_Voxel::EulerStep(double * command = nullptr)
{
	double dt = pSim->dt;
...
//		if(pSim->IsMaxVelLimitEnabled()){ //check to make sure we're not going over the speed limit!
	...
	//std::cout << pSim -> pEnv->getGrowthAmplitude()<<std::endl;
	...
	//double *command = Hopf(pSim->pUserParameter,pSim->UserParameter_num,pSim->CurTime);
	...
	if (pSim->CurTime >= pSim->GetInitCmTime())
	{
		...
		if (pSim->UserParameter_Enable)
		{
			const double pi = 3.14159265358;
			if (segment > 0)
			{
				CtrlTempFact = ThisCTE*(sin(1/6*pi + command[segment-1]*pi/6)+1*command[3])/(1+abs(command[3]))*TempAmplitude;
		...
		//for (int i = 0;i < pSim->UserParameter_num;i++)
			//std::cout<<pSim->pUserParameter[i]<<std::endl;
		
		CtrlTempFact = 0;
		vfloat realTimePeriod,MinTempPeriod,realTime;
		MinTempPeriod = TempPeriod/frequency;
		//std::cout << MinTempPeriod <<std::endl;
		//realTime = pSim->CurTime + MinTempPeriod * phaseOffset  - ((int)(pSim->CurTime/MinTempPeriod))*MinTempPeriod;
		realTime = pSim->CurTime + MinTempPeriod * phaseOffset  - pSim->CurStepCount * TempPeriod;
		//std::cout << "realTime: " << realTime << "MinTempPeriod"<< MinTempPeriod <<"\r";
		realTimePeriod = MinTempPeriod*(1 - peakTime - peakTime_negative - dutyCycle_n2p - dutyCycle_p2n);
		//std::cout << peakTime + peakTime_negative << std::endl;
		//std::cout << dutyCycle_n2p + dutyCycle_p2n<< std::endl;
		if (realTimePeriod < 0)
		{
			dutyCycle_n2p = 0;
			dutyCycle_p2n = 0;
		...
		if (realTime < realTimePeriod/4)
		{
			CtrlTempFact += ThisCTE*TempAmplitude*sin(2*3.1415926f * (realTime/realTimePeriod));
		}
		else if (realTime < realTimePeriod/4 + peakTime*MinTempPeriod)
		{
			CtrlTempFact += ThisCTE*TempAmplitude;
		}
		else if (realTime < realTimePeriod/2 + peakTime*MinTempPeriod)
		{
			CtrlTempFact += ThisCTE*TempAmplitude*sin(2*3.1415926f * ((realTime-peakTime*MinTempPeriod)/realTimePeriod));
		}
		else if (realTime < realTimePeriod/2 + (peakTime+dutyCycle_p2n)*MinTempPeriod)
		{
			CtrlTempFact += 0;
		}
		else if (realTime < 3*realTimePeriod/4 + (peakTime+dutyCycle_p2n)*MinTempPeriod)
		{
			CtrlTempFact += ThisCTE*TempAmplitude*sin(2*3.1415926f * ((realTime - peakTime*MinTempPeriod - dutyCycle_p2n*MinTempPeriod)/realTimePeriod));
		}
		else if (realTime < 3*realTimePeriod/4 + (peakTime + dutyCycle_p2n + peakTime_negative)*MinTempPeriod)
		{
			CtrlTempFact -= ThisCTE*TempAmplitude;
		}
		else if (realTime < realTimePeriod + (peakTime+dutyCycle_p2n+peakTime_negative)*MinTempPeriod)
		{
			CtrlTempFact += ThisCTE*TempAmplitude*sin(2*3.1415926f * ((realTime-peakTime*MinTempPeriod - dutyCycle_p2n*MinTempPeriod - peakTime_negative*MinTempPeriod)/realTimePeriod));
		}
