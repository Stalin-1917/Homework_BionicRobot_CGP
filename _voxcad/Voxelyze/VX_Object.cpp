...
bool CVXC_Structure::ReadXML(CXML_Rip* pXML, std::string Version, std::string* RetMessage)
{
	...
	if (pXML->FindElement("Segment")){
		usingSegment = true;
		int voxCounter = 0;

		InitSegmentArray(X_Voxels*Y_Voxels*Z_Voxels);
		for (int i=0; i<Z_Voxels; i++)
		{
			std::string DataIn;
			std::string RawData;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetSegment(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
		}
	...
	if (pXML->FindElement("DutyCycle_n2p")){ 
		usingPhaseOffset = true;
		int voxCounter = 0;
		// std::cout << "found weights!" << std::endl;
		InitDutyCycle_n2pArray(X_Voxels*Y_Voxels*Z_Voxels);
		for (int i=0; i<Z_Voxels; i++)
		{
			std::string DataIn;
			std::string RawData;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetDutyCycle_n2p(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
	...
	{
		usingDutyCycle_n2p = false;
	}
	if (pXML->FindElement("DutyCycle_p2n")){ 
		usingPhaseOffset = true;
		int voxCounter = 0;
		// std::cout << "found weights!" << std::endl;
		InitDutyCycle_n2pArray(X_Voxels*Y_Voxels*Z_Voxels);
	...
	if (pXML->FindElement("PeakTime")){ 
		usingPeakTime = true;
		int voxCounter = 0;
		// std::cout << "found weights!" << std::endl;
		InitPeakTimeArray(X_Voxels*Y_Voxels*Z_Voxels);
		for (int i=0; i<Z_Voxels; i++)
		{
			std::string DataIn;
			std::string RawData;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetPeakTime(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
	...
	if (pXML->FindElement("PeakTime_negative")){ 
		usingPeakTime_negative = true;
		int voxCounter = 0;
		// std::cout << "found weights!" << std::endl;
		InitPeakTime_negativeArray(X_Voxels*Y_Voxels*Z_Voxels);
		for (int i=0; i<Z_Voxels; i++)
		{
			std::string DataIn;
			std::string RawData;
			...
			for (int k=0; k<X_Voxels*Y_Voxels; k++)
			{
				if (pData[X_Voxels*Y_Voxels*i+k] > 0)
				{
					SetPeakTime_negative(voxCounter,atof(dataArray[k].c_str()));
					voxCounter++;
				}
			}
