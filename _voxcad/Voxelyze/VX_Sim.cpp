...
void Hopf (double *command,double * parameter,int para_num,double time);
...
//		if (pXML->FindLoadElement("Integrator", &tmpInt)) CurIntegrator = (IntegrationType)tmpInt; else CurIntegrator = I_EULER;
	...
	if (pXML->FindElement("UserData"))
	{
		
		UserParameter_num = 0;
		std::string str="Data_";
		while(pXML->FindElement(str.append(std::to_string(UserParameter_num))))
		{
			pXML->UpLevel();
			str = "Data_";
			UserParameter_num++;
		}
		std::cout << UserParameter_num << std::endl;
		if (UserParameter_num > 0)
		{
			UserParameter_Enable = true;
			pUserParameter = new double[UserParameter_num];
		}
		
		for (int i = 0;i < UserParameter_num; i++)
		{
			str = "Data_";
			pXML->FindLoadElement(str.append(std::to_string(i)),&pUserParameter[i]);
			std::cout<< pUserParameter[i] << std::endl;
		}
		pXML->UpLevel();
	}
...
//	for (int k=0; k<iT; k++) BondArrayCollision[k].ResetBond();
	...
	for (int i=0; i<NumVox(); i++) 
	{
		...
		VoxArray[i].frequency = ( pEnv->pObj->GetUsingFrequency() ) ? pEnv->pObj->GetFrequency(i)	: 1.0;
		VoxArray[i].peakTime = ( pEnv->pObj->GetUsingPeakTime() ) ? pEnv->pObj->GetPeakTime(i)	: 0.0;
		VoxArray[i].peakTime_negative = ( pEnv->pObj->GetUsingPeakTime_negative() ) ? pEnv->pObj->GetPeakTime_negative(i)	: 0.0;
		
		VoxArray[i].segment = (pEnv->pObj->GetUsingSegment()) ? pEnv->pObj->GetSegment(i): 0.0;

		VoxArray[i].dutyCycle_n2p = ( pEnv->pObj->GetUsingDutyCycle_n2p() ) ? pEnv->pObj->GetDutyCycle_n2p(i)	: 0.0;
		VoxArray[i].dutyCycle_p2n = ( pEnv->pObj->GetUsingDutyCycle_p2n() ) ? pEnv->pObj->GetDutyCycle_p2n(i)	: 0.0;
        // Initial Volume
...
bool CVX_Sim::TimeStep(std::string* pRetMessage)
{
	...
	if(SelfColEnabled){
		try {UpdateCollisions();} //update self intersection lists if necessary
			...
			std::cout << "Insufficient memory. Reduce model size." <<std::endl;
			if (pRetMessage) *pRetMessage += "Insufficient memory. Reduce model size."; return false;} //catch if we run out of memory
	}
	...
	if (!Integrate()){
		...
		std::cout<< "Simulation Diverged. Please reduce forces or accelerations." <<std::endl;
		return false;
	}
...
//#pragma omp parallel for
	...
	if (CurStepCount%Control_rate == 0)
	{
		...
		Sim_Client.SendBuf_process(u);
		if (Sim_Client.send_buffer()<0)
			return 0;
	...
	for (int i = 0;i<3;i++)
		Record_file_u << u[i]<<" ";
	Record_file_u << CurTime <<std::endl;
	if (CurTime > 3)
	{
		//pUserParameter[3] = 80;
		command[3] = 1;
	...
	Hopf(command,pUserParameter,UserParameter_num,CurTime);
	for (int i=0; i<iT; i++) { VoxArray[i].EulerStep(command);}
...
void CVX_Sim::GetSegmentLocation(double * u)
{
	int index = 0;
	int iT = NumVox();
	double uh[3];
	...
	for (int i = 0;i<iT;i++)
	{
		if (VoxArray[i].GetMaterialIndex() == 2)
		{
			u[0] +=VoxArray[i].GetCurPos().x;
			u[1] +=VoxArray[i].GetCurPos().y;
			u[2] +=VoxArray[i].GetCurPos().z;
			index +=1;
		}
	}
	for (int i = 0;i<3;i++)
...
void Hopf (double *command,double * parameter,int para_num,double time)
{
	...
	static double u[3] = {1,0,-1};
	static double v[3] = {0,-sqrt(2)/2,sqrt(2)/2};
	double du[3] = {0,0,0};
	double dv[3] = {0,0,0};;
	double dt;
	const double pi = 3.14159265358;
	double k,A,phase,f,delta;
	if (para_num == 5)
	{
		k = parameter[0];
		A = parameter[1];
	...
	for (int i = 0;i<2;i++)
	{
		du[i+1] = k*(A - u[i+1]*u[i+1] - v[i+1]*v[i+1])*u[i+1] - 2*pi*f*v[i+1];
		dv[i+1] = k*(A - u[i+1]*u[i+1] - v[i+1]*v[i+1])*v[i+1] + 2*pi*f*u[i+1] + delta*(u[i]*sin(phase)+v[i]*cos(phase));
	}
	du[0] = k*(A - u[0]*u[0] - v[0]*v[0])*u[0] - 2*pi*f*v[0];
	dv[0] = k*(A - u[0]*u[0] - v[0]*v[0])*v[0] + 2*pi*f*u[0];
